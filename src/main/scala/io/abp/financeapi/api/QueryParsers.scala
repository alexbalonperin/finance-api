...
object CustomQueryParsers {
  implicit def nonNetIntParser[F[_]: Monad] = new QueryParser[F, NonNegInt] with RhoDsl[F] {
    override def collect(name: String, params: Params, default: Option[NonNegInt]): ResultResponse[F, NonNegInt] = {
      params.get(name) match {
        case Some(Seq(value: String, _*)) => Try(value.toInt).toEither.flatMap(NonNegInt.from) match {
          case Right(n) => SuccessResponse(n)
          case Left(e) => badRequest(s"Couldn't parse query parameter '$name'. Error: $e")
        }
